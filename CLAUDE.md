...
- **Z-order masonry** (2026-07-11; replaced the brief multi-column lanes, whose down-the-lane order broke the chronology): the wall must read across the columns then down, AND pack tight despite varied card heights. No stable browser has native masonry, so it's the measured-row-span technique: `scripts/grid-masonry.js` (flag-gated `<script>`) spans each card to its measured height on a 1px-auto-row grid (`.is-packed`), and grid auto-placement provides the Z. A `ResizeObserver` per card keeps spans true through brand swaps, media loads, read-more unfolds, and window resizes - no manual hooks. No-JS fallback = aligned rows (correct order, craters accepted; grid view is JS-entered anyway). When native masonry ships, delete the script and switch `.is-packed` to the native declaration.
