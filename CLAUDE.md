...
**The app-ui contract (`[data-ui='app']`) - the chrome is its own app.** (Derek, 2026-07-28; restated harder 2026-08-10 after the Terminal character caused two accidents.) The full contract lives as THE colocated comment at the top of the `[data-ui='app']` block in `styles/modules/settings-panel.css` - read it before touching anything in the chrome. The three laws in short: **(1) form never changes** - no axis may add, remove, swap, resize, or move a control; **(2) the only door in is the `--app-*` color slots** - they hold the chrome's own scheme-aware neutral baseline (2026-08-11; they used to alias the page tokens, which let every theme wash in passively), and chrome color changes only as a deliberate take repainting those slots; **(3) everything else is sealed explicitly** - families pinned, metrics re-stated at the scope, geometry frozen, chrome markup never wears a page voice class (labels are `.app-data-voice`), the scope's airlock re-derives the type ladder at a pinned ratio and restates every voice token at base values, and any NEW global token or axis is assumed to pierce the scope until pinned there. The scope sits on the whole `.site-tray` (header.php) and the settings band (home.php), not just the panels.
