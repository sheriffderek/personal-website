...
- **Known future member: the Pages menu.** When the sub-pages become real (How I work / Now / Contact - the Pages trigger + popover are already built and commented out in `includes/settings-panel.php`), site navigation joins the corner chrome in BOTH forms: the hamburger returns to the list-view toolbox row, and the island gets a menu button (nav passes the membership test everywhere - it's orphaned at every scroll position). Same one-instance popover pattern as the settings member.
