...
- **Membership test for future additions**: is its job orphaned at this scroll position? Members earn a slot by that, not because the island has room.
- **Known future member: the Pages menu.** When the sub-pages become real (How I work / Now / Contact - the Pages trigger + popover are already built and commented out in `includes/settings-panel.php`), site navigation joins the corner chrome in BOTH forms: the hamburger returns to the list-view toolbox row, and the island gets a menu button (nav passes the membership test everywhere - it's orphaned at every scroll position). Same one-instance popover pattern as the settings member. **The hamburger glyph is reserved for this member** - field-tested 2026-07-11: a three-lines (or dots-and-lines) glyph on the island got read as "menu" instantly, twice. That instinct is the asset; only real navigation may wear that glyph, and when the menu ships, the island slot where the layout button briefly lived is its natural home.
