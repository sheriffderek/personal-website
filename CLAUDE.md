...
- `data-variant` on `<article class='milestone'>` — the per-poster ROLE slot (`warm | cool | stone | night | moss | rose`; renamed from `data-flavor` 2026-07-28 — "flavor" now names the color-range axis, see flavors.css). A variant is palette-free; each mood × character cell decides what the slots mean in pigment.
...
**The app-ui contract (`[data-ui='app']`) - the chrome is its own app.** (Derek, 2026-07-28; restated harder 2026-08-10 after the Terminal character caused two accidents.) The full contract lives as THE colocated comment at the top of the `[data-ui='app']` block in `styles/modules/settings-panel.css` - read it before touching anything in the chrome. The three laws in short: **(1) form never changes** - no axis (brand, character, mood, flavor, scheme, or anything future) may add, remove, swap, resize, or move a control; **(2) the only door in is the `--app-*` color slots** - themes repaint those and nothing else; **(3) everything else is sealed explicitly** - families pinned (`--font-body`/`--font-heading`/`--font-ui`), metrics (size/weight/line-height) re-stated at the scope, geometry frozen, chrome markup never wears a page voice class (labels are `.app-data-voice`), the scope's airlock re-derives the type ladder at a pinned ratio and restates every voice token at base values (so even a page voice inside the chrome resolves to the base look), and any NEW global token or axis is assumed to pierce the scope until pinned there. The scope sits on the whole `.site-tray` (header.php) and the settings band (home.php), not just the panels - the toolbar triggers read `--app-*` tokens and were silently unresolved outside it (that's also why the triggers' spec'd circular chrome vanished after the lab port).
...
**FOUC pattern.** Inline `<script>` in `<head>` at [includes/header.php](includes/header.php) reads localStorage keys (`brand-preference`, `emphasis-preference`, `scheme-preference`, and `view-preference` on the timeline page when the grid flag is on) and sets `<html>` attrs BEFORE stylesheets load. Sound is deliberately NOT restored here - `data-sound` gates audio playback only, nothing rendered, so settings-panel.js restoring it at init is early enough. Adding a new persisted axis = adding a line there AND its wiring in [scripts/settings-panel.js](scripts/settings-panel.js) (the `BRANDS`/`EMPHASES` arrays, a `SWITCHERS` entry, or the view section) - the lists can drift silently. The brand/emphasis value lists live in three places that must agree: the FOUC script, the JS arrays, and the sliders' `max` in `includes/settings/{brand,emphasis}-switcher.php`.
