...
- **poster only** → the poster-shapes alone (a card wants a visual but has no slides/videos yet). Opt in with `"poster": true` for the full 16:9 cover, or a purpose-made named size - `"poster": "large"` (16:9, same as true but said out loud), `"medium"` (16:6), or `"small"` (16:3) - for a heading-graphic frame (same poster-shapes tech, cover-cropped to the shorter band). On phones every size relaxes to the 3-wide system (2026-07-28, from the lab): `large`/`true` → 3:3, `medium` → 3:2, `small` → 3:1 (milestone.css). WHICH slice of the art shows is each poster's own call: the `--crop-x`/`--crop-y` tokens in the poster svg's style attribute (its own 1600×900 units, scrubbable in devtools) anchor the window via `scripts/poster-crops.js`; no anchor = centered. **Sizes are authored in the JSON, never derived** from weight or shape - the key IS the record of the design decision. The string value becomes `data-poster-size` on the article; milestone.css maps it to `--media-ratio`.
...
- **Panels are placed by JS** (`placePanel`, settings-panel.js), not CSS anchor positioning (Chrome-only - it broke in Safari, the recruiters' browser). The toolbar's `flex-direction` decides below vs beside; beside prefers the free side using the shared `--layout-panel-max` cap; everything clamps inside the viewport. `--layout-panel-*` tokens in default-layout.css are the single source of the numbers - the JS reads them.
- **Panels are NOT native popovers** (de-popovered 2026-08-11). Open/close, Esc, light-dismiss, and focus return are the manual state machine in settings-panel.js (`openPanelNow`/`closePanelNow`, `.is-open`); the panels' markup lives in `includes/site-panels.php`, included AFTER the tray (its `translateZ(0)` would otherwise become their fixed-position containing block). Why: placement, dim, and iOS dismiss were already manual, and the popover's top layer painted panels above the tray unconditionally - vetoing the perched-trigger design and leaving z-order out of our hands. The stacking is now ours: shade 4 < tray 5 < panel 6. (Note: "panel between shade and tray" is IMPOSSIBLE - position:sticky makes the tray one atomic stacking slab - which is why the perch composition uses replica triggers: clones of the toolbar's menu buttons that each panel carries and placePanel pins over the real ones at < 1024. Tap-identical, aria-hidden, can't drift.) Closed = `display: none` via `.panel:not(.is-open)` - locked, an invisible closed panel eats toolbar taps. No-JS: panels can't open; accepted - the footer's site-map carries navigation.
- **The dim is derived, not declared**: placePanel writes `[data-over]` when the placed panel's rect overlaps `<main>`; the shade (one persistent element, never per-popover `::backdrop`) shows only for an over-content panel, reconciled once per frame so a panel-to-panel switch never blinks. It sits BELOW the tray, so triggers stay clickable over the dim.
