...
- `data-brand-mood` on `<html>` — color: a full palette on the semantic slots (`technical | quiet`). Absent = Expressive, the default. Every mood block restates the FULL 10-token set (the MOOD CONTRACT at the top of `settings/moods.css`). Never touches type or shape.
