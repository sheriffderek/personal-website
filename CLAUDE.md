...
- **poster only** → the poster-shapes alone (a card wants a visual but has no slides/videos yet). Opt in with `"poster": true` for the full 16:9 cover, or a purpose-made named size - `"poster": "large"` (16:9, same as true but said out loud), `"medium"` (16:6), or `"small"` (16:3) - for a heading-graphic frame (same poster-shapes tech, cover-cropped to the shorter band). On phones every size relaxes to the 3-wide system (2026-07-28, from the lab): `large`/`true` → 3:3, `medium` → 3:2, `small` → 3:1 (milestone.css). WHICH slice of the art shows is each poster's own call: `data-crop-x`/`data-crop-y` on the poster's svg (its own 1600×900 units) anchors the window via `scripts/poster-crops.js`; no anchor = centered. **Sizes are authored in the JSON, never derived** from weight or shape - the key IS the record of the design decision. The string value becomes `data-poster-size` on the article; milestone.css maps it to `--media-ratio`.
