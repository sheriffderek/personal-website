...
**The resume system is mission-critical and stands alone (Derek, 2026-09-24).** `/resume/<lane>` and `/resume/<lane>/cover-letter` are what the custom resume PDFs are built from - they must stay clean and very safe. Nothing outside the system feeds them or styles them: its files are the ones listed below, and work on any other page must never touch them. **`/resume` (no lane) is NOT part of the system** - it's a normal page that links into it (`templates/pages/resume.php` + `styles/modules/resume-index.css`, its words written in the template), and it deliberately reads nothing the system is built from (not `content/resume.json`, not `resume.css`). Keep that wall in both directions. **The guard is `bin/resume-fit-check.sh`** (its header comment carries the rules): it exports every lane and approved letter, fails on anything wrong (status, page count, Type 3 fonts, tag tree, missing name/role, lane lists out of sync, theme leaks via `bin/resume-theme-check.mjs`), and publishes to the kit only on a pass - and only when a system file changed; sheets that change with no system change fail as outside drift. The project hook runs it after any edit to a file the resume pages are built from or load; run it by hand before sending anything. **The sheet is theme-proof by pin** - the "THE SHEET IS THEME-PROOF" block in `resume.css` restates every theme token at the approved Product x Expressive x light value (learned 2026-09-24: `DEFAULT_MOOD` going Quiet had silently re-inked the PDF headings).

`/resume/<lane>` - the three-lane resume (Product Designer / Design Engineer / Advocate & Educator), and its one-page PDF exports. **The lane page is deliberately a one-off document posture** - it opts out of the shell's desktop grid (the page IS the resume, the tray overlays as a plain sticky bar); don't "fix" it back into the page-with-sidebar system. Where the truth lives:
...
- "The Web" is capitalized when it names the medium/platform itself ("I got into The Web", "how The Web actually works"). Adjectival uses stay lowercase ("the web side of things", "web design").
- The PE program name is always written Design for <em>The Web</em> - capital The Web, italics on "The Web" only, never on the whole title. The DFTW acronym stays plain.
...
- **poster only** → the poster-shapes alone (a card wants a visual but has no slides/videos yet). Opt in with `"poster": true` for the full 16:9 cover, or a purpose-made named size - `"poster": "large"` (16:9, same as true but said out loud), `"medium"` (16:6), or `"small"` (16:3) - for a heading-graphic frame (same poster-shapes tech, cover-cropped to the shorter band). On phones every size relaxes to the 3-wide system (2026-07-28, from the lab): `large`/`true` → 3:3, `medium` → 3:2, `small` → 3:1 (milestone.css). On phones, `scripts/poster-crops.js` swaps each poster's `viewBox` to a per-poster phone frame defined in its `PHONE_FRAMES` lookup table - the viewBox can extend past the 1600×900 canvas in any direction (zoom out, negative origin, whatever the composition needs); the CSS ground fills the overflow. Posters not in the table stay at the full `0 0 1600 900` frame. **Sizes are authored in the JSON, never derived** from weight or shape - the key IS the record of the design decision. The string value becomes `data-poster-size` on the article; milestone.css maps it to `--media-ratio`.
...
**$todo — no `story` is authored yet; the type is built and unused.** The first one is waiting: `content/milestones/2026-job-search/03-loop-wide.mp4` is a 25-second clip with real narration in its audio track (verify any time with `ffmpeg -i <file> -af volumedetect -f null -` — a mean around -32dB is a voice; the `list-at-ease` loop reads -91dB, i.e. a genuinely empty track). Promoting it = rename the wide + square cuts to `03-story-wide.mp4` / `03-story-square.mp4` and change that item's `"type"` to `story` in `content/milestones.json`. Deliberately not done yet (2026-07-14) - it lands with the grid work, so the wall's motion and the Listen button get judged together.
...
**Why SVG posters, not HTML/CSS compositions (the tradeoff, 2026-08-14).** The poster art is SVG because the compositions need shapes HTML can't make - triangles, circles, diagonal lines, arcs. HTML/CSS gives you rectangles (and rounded rectangles) with pixel-stable borders; SVG gives you any shape, but `stroke-width` scales with the viewBox. That means strokes that look right at desktop width get thinner on phones (the SVG shrinks, and so do its lines). The viewBox reframing system (phone-specific viewBox per poster via `poster-crops.js`) makes this more pronounced: zooming out increases the coordinate-space-to-pixel ratio, thinning strokes further. The line-weight tokens (`--line-width-primary` / `--line-width-secondary` in milestone.css) could be bumped at phone widths via a media query to compensate, but it's approximate - different posters at different zoom levels would want different values. `vector-effect="non-scaling-stroke"` is the nuclear option (locks strokes to CSS pixels, ignoring viewBox scaling) but kills the proportional relationship between stroke and composition. **The accepted tradeoff: SVG earns its place for shape vocabulary; line-weight scaling is the tax.** Bump the tokens per-breakpoint if it ever gets bad enough to fix; don't rebuild posters as HTML to solve it.
...
- It gets a reserved (transparent) slot in the toolbar, never pushing the settings button sideways. Chrome that jumps mid-scroll is worse than the problem it solves.
...
**The tray's reveal members (lab port 2026-07-19 - replaced the corner island).** The 2026-07-11 corner island (a separate floating cluster that appeared when the grid's inline panel scrolled away) is retired: the `site-tray` is a sticky column that rides the page and pins near the top, so the "always reachable corner chrome" job is the tray's own - no second floating element to maintain. Its conditional-membership idea survives as **reveal members**: settings and back-to-top hide at ≥1450 grid while the settings band is on screen, and rejoin the toolbar when `data-scrolled` flips (the band's IntersectionObserver in settings-panel.js - band-exit IS the condition, no magic scroll depth). Rules that carry over:
- **Membership test for future toolbar members**: is its job orphaned at this scroll position? Members earn a slot by that. NO layout-switch member - every glyph for it read as a menu (two attempts, 2026-07-11), and the settings popover already carries the Layout row one tap away.
- **Now-playing pill - probably NOT a grid member** (2026-07-11): the wall is dense and easy to navigate back through, so orphaned audio barely orphans there. The pill is a list-view concern (35 cards of single column between you and the talking video).
- **The Pages menu is LIVE and PRIMARY** (2026-07-19 - built now on purpose, so the shell is proven with two panels; if the nav turns out premature, removing it is one block in `includes/settings-panel.php`, not a redesign). First in the toolbar's DOM (importance order - the `row-reverse`/column axis lands it corner-most), on every page, at every scroll depth, never a reveal member. `#pages-menu` wraps the `includes/settings/page-menu.php` nav (aria-current, carries `?target=` forward). **The hamburger glyph is reserved for this member** - field-tested 2026-07-11: a three-lines glyph got read as "menu" instantly, twice. Only real navigation may wear it.
- **The mirror model is BUILT** (was the "prerequisite for settings appearing in more places"): the settings rows are one shared partial (`includes/settings-rows.php`) mounted N times (panel + band, per-instance `id_suffix` for the label ids); settings-panel.js reflects all instances from `data-*` on `<html>` + localStorage, and none owns the state. A settings cell at the wall's end (if ever) additionally needs masonry manners: always last, excluded from filter/minimap counts, pinned so `dense` can't backfill it.
...
**The sizing rule (Derek, 2026-08-11) — prove the goal, or simplify.** The system exists to do two jobs: (1) the default look makes Derek read as established with the least necessary; (2) the panel convinces the designers/devs it gets passed to that he knows his stuff. Every proposed addition gets judged against those: the goal is never "do whatever happens" — it's to *complete a system that proves the goal*. If the system grows so big that combos stop proving a point, we're losing track of more things than matter. Default to simplifying; additions must earn their multiplication (a new axis value multiplies states, QA surface, and copy risk). Current size: 5 characters × 3 moods × 4 flavors × 2 schemes ≈ 120 states from ~25 authored decisions — that ratio IS the pitch, and it only stays true while the discipline holds.

**The model (settled 2026-07-28, replacing the earlier brand/emphasis pair): character = structure, mood = color policy, flavor = pigment routing.** Any character works under any mood under either scheme, because components only ever read the semantic slots. The five characters are *registers one design system serves* — the pitch is one system carrying a company's real surfaces. **Authority is the cascade-layer order** declared in `styles/settings.css` (`base, characters, moods, scheme, variants, flavors, red-light`) — a later layer beats an earlier one regardless of specificity, so red-light stomps everything and specificity arms races are over.

**Attribute axes (each lives at a specific level, do not confuse):**
- `data-brand-character` on `<html>` — structure: type pair, `--corners`, scale rhythm (`marketing | interface | editorial | terminal`). Absent = Product, the default (`:root` in `settings/base.css` IS Product). Never names a hue. Blocks are deltas-only; unstated tokens fall through to Product.
- `data-brand-mood` on `<html>` — color: a full palette on the semantic slots (`technical | quiet`). Absent = Expressive, the CSS default. (The mood a new visitor STARTS in is a separate knob: `DEFAULT_MOOD` in `includes/config.php`, matched in `scripts/settings-panel.js` - the why lives in the config comment.) Every mood block restates the FULL 10-token set (the MOOD CONTRACT at the top of `settings/moods.css`). Never touches type or shape.
- `data-flavor` on `<html>` — the color-range axis (`earth | cool | sweet`; absent = the house default): what the variant slots MEAN in pigment, per mood × character cell. See `settings/flavors.css` — including the index-0 law (default axis values are never written as attributes; the JS removes the attribute instead).
- `data-scheme` on `<html>` — `system | light | dark`. Absent = system (reads `prefers-color-scheme`). One `light-dark()` pair per token, so there are no separate dark blocks to stomp a mood.
	- **Three states, deliberately - do not "correct" this to a two-state toggle.** Common web guidance (including the Chrome team's dark-mode guide) says to expose only two states, on the reasoning that a toggle is a comfort knob and "system" is a redundant option most users can't reason about. That reasoning doesn't hold here: on this site the settings panel IS the design-system pitch, and the visitor poking at it is being *shown the mechanism*. The system row (labeled "Auto" in the panel, 2026-08-14) is the one that teaches them their OS has its own setting, and that a page can defer to it or override it within its own scope. Collapsing to two states would hide exactly the thing the panel exists to demonstrate. (Under `light-dark()`, system = `color-scheme: light dark` — literally "defer to the OS.")
- `data-red-light` on `<html>` — boolean override (bare attribute = on) that stomps ALL color; the theatrical finale, its own panel row, not a stop on the mood dial. Last cascade layer, wins forever (`settings/red-light.css`).
...
- `data-variant` on `<article class='milestone'>` — the per-poster ROLE slot (`warm | cool | stone | night | moss | rose`; renamed from `data-flavor` 2026-07-28 — "flavor" now names the color-range axis). A variant is palette-free; the flavor decides what each role means in pigment.
- `data-ui='app'` on the persistent chrome — the chrome-stays-stable scope, see the app-ui contract below.

**A character block never touches color; a mood block never touches type or shape.** That one-way split is what makes the axes composable — a `color:` in a character block or a `--font-*` in a mood block is wrong by definition. (The one documented exception: Marketing's white paper lives in `flavors.css` with its reasoning.)

**Font pairings are placeholders** until the Figma audition locks them (Product = Quicksand/Spline Sans is the current live look; the other characters carry working stand-ins). Swap families in the character blocks in `styles/settings/characters.css`; the render-blocking primary pair `<link>` in `includes/header.php` must stay matched to the Product pair. After locking, trim the unused audition families from the async font `<link>`.

**Corners.** `--corners` is character-owned (declared per character block) and consumed by the milestone media frame (`milestone.css`, `var(--corners, 0)`). The settings panel keeps its own `--app-corners` on purpose, so character-switching never morphs the panel.

**Why moods restate the full set (no indirection).** We tried a `--t-*-light` / `--t-*-dark` intermediate layer. It caused inheritance mysteries in top-layer popovers and a "not defined" tooltip on `--stroke-primary` that took hours to unwind. Simpler wins: every mood declares all 10 semantic tokens directly, `light-dark()` per token. The tradeoff (a new color token = editing every mood block) is accepted — a dropped token is visible at a glance, which is the point of the contract.

**The app-ui contract (`[data-ui='app']`) - the chrome is its own app.** (Derek, 2026-07-28; restated harder 2026-08-10 after the Terminal character caused two accidents.) The full contract lives as THE colocated comment at the top of the `[data-ui='app']` block in `styles/modules/settings-panel.css` - read it before touching anything in the chrome. The three laws in short: **(1) the box never changes** - no axis may add, remove, resize, or move a control, or change spacing or fonts; anything painted on a same-size box may change (shape, color, gradient, panel border/shadow/corners, same-size icon swaps - Derek, 2026-09-25); **(2) the only door in is the `--app-*` open bank** (color, shadow, corners) - it holds the chrome's own scheme-aware neutral baseline (2026-08-11; the slots used to alias the page tokens, which let every theme wash in passively), and the chrome changes only as a deliberate take repainting those slots; the page is the brochure, the chrome is where UI craft shows, in named patterns (the vocabulary is in the CHROME TAKES comment); **(3) everything else is sealed explicitly** - families pinned, metrics re-stated at the scope, geometry frozen, chrome markup never wears a page voice class (labels are `.app-data-voice`), the scope's airlock re-derives the type ladder at a pinned ratio and restates every voice token at base values, and any NEW global token or axis is assumed to pierce the scope until pinned there. The scope sits on the whole `.site-tray` (header.php) and the settings band (home.php), not just the panels.
...
- **Large screens** - it gains a **fake settings panel** to the right and becomes **two columns**, because the real layout gains the tray there. The fake panel is the diagram's way of showing "the page is two-column now."
...
**Why the density is load-bearing (Derek, 2026-09-16 - two audiences, both served by the same artifact).** The grid does double duty and neither reason works alone; kept together they lock it.
- **Recruiter-facing: it denies the two-second pattern-match.** A standard portfolio's hero shot trips "is this our aesthetic?" in the first two seconds - one image of a banking product and a health-tech recruiter thinks "but we're not a bank" and closes the tab. No single card in the wall is big enough to be *the look*, so the eye can't shortcut and has to actually read a card to know what it's looking at. The grid is an anti-pattern-match device; the friction of reading IS the feature.
- **Designer/dev-facing: it's the surface the theme system performs on.** A dense field of small varied cards is where a token system actually shows its work - one hero image would barely register a character change, 35 cards re-painting in unison is the demo. Grain and variety across cards is the point, not a problem to smooth out.
- **The lock:** a cleaner or sparser grid weakens both jobs at once - less friction to scan past, less surface for the theme to perform on. When someone (future us, a copywriter, a reviewer) suggests "make it cleaner / more uniform / lead with the best one" it will sound reasonable in isolation - it isn't. The read-more copy is where the actual persuasion happens; the grid is the mechanism that gets them there, which is why the copy bar stays high (a card that gets opened and reads thin undoes the whole setup).

**The rules:**
- Grid exists only at **≥ 1200px** (the lab port, 2026-07-19; was 1600). Below that the toggle hides and everyone gets the list — a phone gridding into one column would just be a worse list. A saved grid preference persists but applies only where the grid exists (`data-view='grid'` on `<html>` reflects the APPLIED state, not the preference; the media-query gate lives in the FOUC script, `GRID_MIN` in settings-panel.js, and grid-view.css — keep the three matched).
- **Columns**: 2 from 1200px, 3 from 1600px. The wall's geometry is shared `--layout-wall-*` tokens in grid-view.css — `--layout-wall-columns` (count), `--layout-wall-lane` (the designed lane measure; cards never stretch past it), `--layout-wall-gap` (gutter), `--layout-wall-max` (the derived whole-wall width), plus `--layout-tray-*` for the tray's reserved column from 1450. The wall's max-width derives from count × measure, so a bigger window adds a lane rather than inflating cards; the minimap reads `--layout-wall-columns`. (Structure-critical measures wear the `--layout-<system>-<measure>` prefix site-wide — the list/sidebar/panel set lives in default-layout.css.)
- **Cells keep the full card** (date, title, media, description, read-more) at column width; only motion stands down.
- **The lane dealer** (2026-07-12; replaced BOTH earlier masonries - the strict i-mod-N deal whose height-blind thirds dumped their whole mismatch at the bottom edge, and the measured-row-span packer whose set-every-height approach meant owning every height change forever, with re-packs shifting the whole wall under the reader). The principle: **distribute once, then let the page flow**. `grid-masonry.js` deals the cards into N real lane wrappers (`.timeline-lane`, `role='presentation'`, flex column stacks via `.is-laned` in grid-view.css) - after the deal there are no spans, no observers, no repacking: a read-more unfolds and pushes only its own lane down naturally; the wear from reading is honest and stays. The deal itself: cards in source order, round-robin left-to-right, but a card diverts to the shortest lane when the cycle's lane is taller than it by a tolerance - and since no single tolerance suits every card set, the dealer tries several (pure arithmetic, no DOM) and keeps the deal with the tightest bottom edge (ties go chronological). Verified ~80px spread vs ~2000px for the blind deal. Re-deals happen only at deliberate setup moments: entering grid view, `timeline:filtered` (announced by applyFilter), lane-count changes at 1600px, and `document.fonts.ready` (pre-font heights mislead the deal ~15x). Leaving grid view hands every card back to the one `<ol>` in source order. Card heights are read once to steer assignment, never written. Text length is content truth and is never clamped to fix geometry. No-JS fallback = the base aligned-rows grid (correct order, craters accepted; Derek approved it as fallback-only). JS that finds timeline items must use both depths: `.timeline > li, .timeline > .timeline-lane > li` (bare `.timeline li` catches list items inside card content). When native masonry ships, the wrappers delete and the `<ol>` becomes the grid.
- **Grid is home-page-only**: the invite, the Layout row, and the FOUC `data-view` line all gate on the page carrying the timeline controls (`$page_controls === 'filter-control'`) - on any other page they'd be doors to nothing.
- **The grid invite**: a small circular toggle beside the settings trigger (markup in settings-panel.php, flag-gated) that exists only in list view at ≥1200px and pulses an accent halo until the visitor enters grid view once by any door — then a `grid-invite-seen` localStorage breadcrumb retires the pulse forever (same pattern as the passkey button). Respects `prefers-reduced-motion`. It is the door IN only — the way back out of grid is the settings panel's Layout row, by design.
...
- **Panels are NOT native popovers** (de-popovered 2026-08-11). Open/close, Esc, light-dismiss, and focus return are the manual state machine in settings-panel.js (`openPanelNow`/`closePanelNow`, `.is-open`). Why: dim and iOS dismiss were already manual, and the popover's top layer painted panels above the tray unconditionally - vetoing the docked-under-the-circles design and leaving z-order out of our hands. Closed = `display: none` via `.panel:not(.is-open)` - locked, an invisible closed panel eats toolbar taps. No-JS: panels can't open; accepted - the footer's site-map carries navigation.
- **The dim is derived, not declared**: `syncOverContent` (settings-panel.js) writes `[data-over]` when the open panel's rect overlaps `<main>` - checked on open and re-checked by the slim reconcile (width resize, view switch, band observer; a same-width list↔grid toggle moves the panel with no resize, which is why the reconcile re-derives it). The shade (one persistent element, never per-popover `::backdrop`) shows only for an over-content panel, reconciled once per frame so a panel-to-panel switch never blinks. It sits BELOW the tray, so triggers stay clickable over the dim.
- **The phone posture is a set of facts, not a style** (Derek, 2026-09-25): a fixed header is a given; the trigger icons never float over content (they rest in the full-width bar); the panel is a card, never a full-screen takeover - its edges are what the chrome's treatments paint, and the page must stay visible so a visitor watches it repaint. The reasoning, and the open question about the dim, is THE PHONE POSTURE block on `.panel` in `styles/modules/settings-panel.css`.
- **Locked rules in force**: a panel may not outlive its trigger · an open panel stays open across a width-resize and re-places live · DOM order is importance order, primary corner-most (`row-reverse` in row situations, plain column in column ones) · the grid button is the door IN only.
...
Three toys worth planning someday. Written down for the *shape*, not because any is queued. None are for Round 1.

**The shared groundwork: an answer bank.** Authored answers to the skeptical questions - STAR-shaped stories plus straight talk. Doesn't exist yet, and it's the actual work; Toys 1 and 2 are thin skins over it (Toy 3 skips it - its corpus is the token system). It pays for itself even if neither ever ships: it's interview prep, cover-letter source, and target-note raw material. Seed it from questions Derek has really been asked in screens, not invented ones.
