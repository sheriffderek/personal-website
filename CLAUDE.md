...
- **poster only** → the poster-shapes alone (a card wants a visual but has no slides/videos yet). Opt in with `"poster": true` for the full 16:9 cover, or a purpose-made named size - `"poster": "large"` (16:9, same as true but said out loud), `"medium"` (16:6), or `"small"` (16:3) - for a heading-graphic frame (same poster-shapes tech, cover-cropped to the shorter band). On phones, `large`/`true` square up like every full cover; `medium`/`small` are heading graphics and KEEP their ratio at every width (encoded via selector specificity in milestone.css - deliberate, don't "fix" it). **Sizes are authored in the JSON, never derived** from weight or shape - the key IS the record of the design decision. The string value becomes `data-poster-size` on the article; milestone.css maps it to `--media-ratio`.
