...
- **Known future member: the Pages menu.** When the sub-pages become real (How I work / Now / Contact - the Pages trigger + popover are already built and commented out in `includes/settings-panel.php`), it takes the PRIMARY slot: first in the toolbar's DOM (importance order - the `row-reverse`/column axis lands it in the corner-most position), on every page, at every scroll depth. **The hamburger glyph is reserved for this member** - field-tested 2026-07-11: a three-lines glyph got read as "menu" instantly, twice. Only real navigation may wear it.
