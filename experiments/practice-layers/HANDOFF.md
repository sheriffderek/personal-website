...
- **How the story hands over to the finale (his storyboard, five panels left to right):** (1) the full stack of names with the last section; (2) scroll past it - the names leave off the top as ONE piece while the stripes keep running down the left on their own, "revealing the layers on the left"; (3) just the thin stripes, tops aligned, bottoms stepped; (4) they widen, the frame lands, the closing line arrives; (5) **not built yet:** "then as we scroll - we can use those to show it a few ways" - his fifth panel turns the same layers into a horizontal stack (first layer at the bottom) with a tall frame cutting down through all of them. Panels 1-4 are built: the names exit together (checked: every name moves by exactly the same amount), each stripe's tail ends exactly where the finale begins, and each thin finale column starts at the same x as its stripe. An earlier attempt let the names fold away one under another - rejected ("sorta").
