/* Sticky-tray border. A zero-height sentinel sits just above the tray.
   When it scrolls out of the top of the viewport, the tray has reached
...
(function () {
	var sentinel = document.querySelector('.tray-sentinel');
	var tray = document.querySelector('.site-tray');
	if (!sentinel || !tray) {
		return;
	}

	var observer = new IntersectionObserver(function (entries) {
		/* sentinel no longer visible = tray is pinned to the top. The
		...
		tray.classList.toggle('is-stuck', !entries[0].isIntersecting && window.scrollY > 0);
	});
