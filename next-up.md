...
- [ ] **Device stage - parked, not pressing** (Derek, 2026-09-19). On large screens, put the site in a phone/tablet-width stage so a desktop visitor sees the small-screen design without resizing or losing their place. Iframe ruled out (loses their spot); the route is container queries, page keeps scrolling. Audited by an independent agent and reviewed - everything we learned, the hazards, and the one-hour experiment to run first live in `notes/device-stage-plan.md`.
